...
//Services
import { createUser } from '../../sevices/Auth/AuthServices'
...
const SignUp = () => {
  ...
  const handleSubmit = async (event) => {
    const { username, companyName, email, password, confirmPassword } = state
    ...
    try {
      if (
        ...
        const authresponse = await createUser(state)
        if (authresponse?.users?.message) {
          alert(authresponse.users.message)
  ...
  const handleChange = (evt) => {
    ...
    if (evt.target.name === 'email' && !EMAIL_REGEX.test(value)) {
      ...
      evt.target.name === 'companyName' &&
      !COMPANY_NAME_REGEX.test(value)
    ) {
      errorFor = 'companyName'
    } else if (evt.target.name === 'username' && !USERNAME_REGEX.test(value)) {
      errorFor = evt.target.name
    }
    setState({
      ...state,
